...
| 9 | [Workload Debugging Basics](PHASE9.md) | Started | Workload Overview includes event-aware debug summaries, `kubectl`-backed port-forward sessions, and external-terminal Pod exec actions |
...
1. Continue Phase 9 by polishing exec/port-forward ergonomics: terminal preference, shell picker, local-port conflict detection, and lightweight exec history.
