...
| 12 | [AI Foundations](PHASE12.md) | In progress | Provider settings, Keychain secret storage, model discovery, OpenAI `max_completion_tokens` compatibility, streaming provider transport tests, redacted resource context, resizable resource-scoped AI assistant window, Markdown/code rendering, top-level AI status page, visual gating QA, and manual AI evaluation checklist are in place |
