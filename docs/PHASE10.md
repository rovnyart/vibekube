...
- 2026-06-21: Added first-class resource Actions from the detail header in a dedicated sheet for scale, rollout restart, typed-confirm delete, stronger namespace delete confirmation, and local per-resource action history. Added a header Apply YAML sheet with a reusable highlighted YAML editor, Cmd+F search, file open, side-by-side server-side dry-run preview/diff before apply, and structured Namespace/ConfigMap/Secret generators.
