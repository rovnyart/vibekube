...
private struct ExecLaunchesButton: View {
    @EnvironmentObject private var appModel: AppModel
    @State private var isPresented = false

    var body: some View {
        if !appModel.execLaunches.isEmpty {
            Button {
                isPresented.toggle()
            } label: {
                Label(buttonTitle, systemImage: "terminal")
            }
            .buttonStyle(.bordered)
            .popover(isPresented: $isPresented, arrowEdge: .bottom) {
                ExecLaunchesPopover()
            ...
            .help(buttonTitle)
            .accessibilityIdentifier("toolbar.execLaunches")
        }
    }

    private var buttonTitle: String {
        let failedCount = appModel.execLaunches.filter { launch in
            if case .failed = launch.status {
                return true
            }
            return false
        }.count
        ...
        if failedCount > 0 {
            return failedCount == 1 ? "1 Exec Failed" : "\(failedCount) Exec Failed"
        }

        return appModel.execLaunches.count == 1 ? "1 Exec" : "\(appModel.execLaunches.count) Exec"
    }
}

private struct ExecLaunchesPopover: View {
    @EnvironmentObject private var appModel: AppModel
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            ScrollView {
                VStack(spacing: 0) {
                    ForEach(appModel.execLaunches) { launch in
                        ExecLaunchRow(launch: launch)

                        if launch.id != appModel.execLaunches.last?.id {
                            Divider()
                        }
                    }
                }
            }
            .frame(width: 460, height: min(340, CGFloat(max(appModel.execLaunches.count, 1)) * 74))
        }
        .frame(width: 460)
    }
}

private struct ExecLaunchRow: View {
    ...
    var body: some View {
        HStack(spacing: 10) {
            ...
            VStack(alignment: .leading, spacing: 4) {
                HStack(spacing: 6) {
                    Text(launch.displayTarget)
                        .font(.callout.weight(.semibold))
                        .lineLimit(1)
                        .truncationMode(.middle)

                    Text(launch.displayNamespace)
                        .font(.caption.weight(.medium))
                        .foregroundStyle(.secondary)
                }
    ...
    private var detailText: String {
        ...
        case .opening:
            "\(launch.displayCommand) opening in \(launch.terminalApp.title)"
        case .opened:
            "\(launch.displayCommand) opened in \(launch.terminalApp.title)"
        case .failed(let message):
            message
        }
    }
